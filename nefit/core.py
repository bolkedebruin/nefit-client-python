...
class AESCipher(object):
    def __init__(self, magic, access_key, password):
        self.bs = 16
        self.key = (
            hashlib.md5(bytearray(access_key, "utf8") + magic).digest()
...
class NefitCore(object):
    ...
    def __init__(
        ...
        self.client = ClientXMPP(
            jid=jid, password=self._accesskey_prefix + access_key, sasl_mech=sasl_mech
        )
    ...
    def message(self, msg):
        if msg["type"] in ("chat", "normal"):
            ...
            self.event.set_result(response)

    def connect(self, block=False, use_ssl=None, use_tls=None, **kwargs):
        self.client.connect(
            use_ssl=use_ssl if use_ssl is not None else self.use_ssl, **kwargs
        )
        self.process(10)
    ...
    def process(self, timeout=None):
        self.event = Future()
        self.loop.run_until_complete(asyncio.wait([self.event], timeout=timeout))
    ...
    def get(self, uri, timeout=30):
        self.send(f"GET {uri} HTTP/1.1\rUser-Agent: NefitEasy\r\r")
    ...
    def send(self, body):
        ...
        str_data = tostring(
            message.xml,
            xmlns=message.stream.default_ns,
    ...
    def get_location(self):
        return self.get("/system/location/latitude"), self.get(
    ...
    def get_usage(self, page):
        return self.get(f"/ecus/rrc/recordings/gasusage?page={page}")

    def get_usage_pages(self):
        d = self.get("/ecus/rrc/recordings/gasusagePointer")
        d["pages"] = ceil(d["value"] / 32)
        return d

    def set_temperature(self, temperature):
        self.put(
            "/heatingCircuits/hc1/temperatureRoomManual", {"value": float(temperature)}
        )
...
class NefitClient(NefitCore):
    ...
    def get_program(self):
        ...
        return {"active_program": data[0]["value"]}
